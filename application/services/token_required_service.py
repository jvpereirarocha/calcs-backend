...
def token_required(func):
    ...
    def decorated(*args, **kwargs):
        ...
        if 'Authorization' in request.headers:
            parts_of_token = request.headers['Authorization'].split(' ') # format must be 'Bearer <token>'
        ...
        try:
            user = User.decode_token_and_get_user_information(token=token, secret_key=getenv('JWT_SECRET_KEY'))
        except:
