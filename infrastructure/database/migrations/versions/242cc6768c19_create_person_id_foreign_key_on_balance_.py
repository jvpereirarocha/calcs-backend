...
def upgrade():
    ...
    op.create_unique_constraint("uq_balance_month_year_person_id", "balances", ["month", "year", "person_id"])
    op.create_index("idx_balance_month_year_person_id", "balances", ["month", "year", "person_id"])
    op.create_index('idx_balance_person_id', 'balances', ['person_id'])


def downgrade():
    op.drop_index("idx_balance_month_year_person_id", table_name="balances", if_exists=True)
